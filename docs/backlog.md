...
## Sprint 9 — Planned
- [ ] dev-loop-rebuild-friction: Editing `src/*.ts` requires three steps before changes take effect — `npm run build` (tsc → dist/), `/mcp` reconnect (respawn server), and remembering to do the build first. Skipping the build silently respawns the OLD compiled JS, masking the change. This bit the dogfooding workflow twice today after PR #13 (stdin) and PR #14 (permissions). Three options considered: (A) background `npx tsc --watch` — smallest change, no per-reconnect overhead but easy to forget the watcher; (B) `.mcp.json` bash wrapper that runs `npm run build` before launching node — eliminates the missed-build failure mode at ~3-5s per reconnect; (C) switch to `tsx` for direct TS execution — eliminates the entire build-output category. (B) adopted as interim local config 2026-04-28. (C) recommended for proper fix — needs `tsx` dev dep, `.mcp.json` template update, and CI verification — source: dogfooding session 2026-04-28
