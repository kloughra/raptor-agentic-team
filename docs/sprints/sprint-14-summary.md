...
- Tests: jest / ts-jest, colocated unit + `tests/integration/`
- Language/runtime: TypeScript / Node.js
- Heading scan, normalization, fence tracking: plain string/regex ops in `retro.ts`
- File I/O: Node `fs` (sync, matching current call sites)
- Git commit: `simple-git`
- State persistence: existing `~/.raptor/{slug}/sprint-N.json` via `saveSprintState`
- Tests: jest/ts-jest — colocated unit (`retro.test.ts` additions) + `tests/integration/retro-improvements-not-applied.integration.test.ts`
- Detection: Package manifest existence
- Scoping: Task description injection
- Config: Existing `config.json`
- Language / runtime: TypeScript on Node.js
- Retry mechanism: Plain bounded `do/while` in `runner.ts` (mirrors Sprint 6 in-process-loop decision)
- Git / merge: `simple-git` + `gh` CLI fallback inside `executeMerge`
- State persistence: JSON sprint-state files via `loadSprintState`/`saveSprintState`
- Failure metadata: Pure functions from `failure-classification.ts` (Sprint 12)
- Tests: jest / ts-jest; seam tests in `tests/integration/`, BDD in `tests/bdd/`
...
- push-before-merge: Push the feature branch to the remote before `gh pr merge` inside `executeMerge`, failing the attempt cleanly if the push fails — kills the Sprint 12 merge failure (branch divergence from unpushed local demo/retro commits) at its root cause instead of retrying it. Architect ruling (Sprint 13 tech-approval, Anky): small (~10 lines) but it changes `executeMerge` mechanics, which were spec Out of Scope for `sprint-completes-despite-failed-merge`; needs tests for push failures, no-remote repos, and force-push hazards. **Sprint 14 candidate.** — source: Sprint 13 architecture OQ1 ruling + demo flag 2026-07-06 (filing directed at tech-approval checkpoint; filing confirmed requested again in feature-2 demo feedback 2026-07-07)
- billing-error-signature-class: Spend-limit errors ("You've hit your monthly spend limit") burned 2 attempts before the no-progress short-circuit fired — three separate times in Sprint 13 alone (feature-1 step 4, commit 908bf63; feature-2 step 3, commit 9394bdd; feature-2 step 7, commit f9bc035). Add a dedicated **user-actionable/billing** classification in `failure-classification.ts` that escalates after 1 attempt: retrying can never succeed until the user acts (raise the limit at claude.ai/settings/usage), so every extra attempt is pure waste. Distinct from `transient` (retry helps) and `deterministic` (task is wrong) — this is a third class where the blocker is outside the sprint entirely. — source: Sprint 13 demo feedback (stakeholder directive to PO at step 10) 2026-07-07
